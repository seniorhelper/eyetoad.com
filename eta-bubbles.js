...
var RM=window.matchMedia&&matchMedia('(prefers-reduced-motion: reduce)').matches,R=Math.random,dpr=Math.min(2,window.devicePixelRatio||1),ctx=cv.getContext('2d');
var ST=[[0,[56,189,248]],[.18,[125,211,252]],[.34,[250,224,50]],[.5,[255,178,0]],[.64,[255,122,0]],[.78,[255,45,111]],[.9,[219,39,119]],[1,[124,58,237]]];
function col(t){t=Math.max(0,Math.min(1,t));for(var i=1;i<ST.length;i++){if(t<=ST[i][0]){var a=ST[i-1],b=ST[i],f=(t-a[0])/(b[0]-a[0]);return a[1].map(function(v,k){return Math.round(v+(b[1][k]-v)*f);});}}return ST[7][1];}
function qc(a){return a.map(function(v){return Math.max(0,Math.min(255,Math.round(v/6)*6));});}
var W=0,H=0,HF=0,B=[],cache={},FX=[],vis=true,last=0,big=[];
...
function pack(){var C=[],G={},cs=80,sc=Math.min(1.4,Math.max(1,Math.sqrt(W/390))),mx=30*sc,tilt=(R()-.5)*.5,wx=.55+tilt,wy=.45-tilt,shift=(R()-.5)*.12,tries=Math.min(90000,Math.round(26000*W/390/sc));
 ...
 B=C;big=B.filter(function(b){return b.r>7&&b.a>.9;});}
function draw(t){ctx.clearRect(0,0,W,H);var ts=t/1000;
 for(var i=0;i<B.length;i++){var b=B[i],sc=1;if(b.pop)continue;if(b.back){var k=Math.min(1,(t-b.back)/700);sc=k<1?1-Math.pow(1-k,3):1;if(k>=1)b.back=0;}
...
document.addEventListener('pointerdown',function(e){if(e.target.closest&&e.target.closest('a,button,input,textarea,select,label,form,svg,.h8-form'))return;var y=e.pageY,x=e.pageX;if(y>H)return;var best=null,bd=1e9;
 for(var i=0;i<B.length;i++){var b=B[i];if(b.pop)continue;var d=Math.hypot(x-b.x,y-b.y);if(d<b.r+4&&d<bd){bd=d;best=b;}}pop(best,performance.now());});
