...
(function(){
  ...
  function land(){
    ...
    box.classList.add('eta-live','eta-fly');
    box.setAttribute('aria-hidden','false');
    track('mascot_appear', {page_path: location.pathname});
    clearTimeout(talkT);
    talkT = setTimeout(function(){
      box.classList.add('eta-talking');
  ...
  function leave(){
    ...
    box.classList.add('eta-leaving');
    clearTimeout(talkT);
    setTimeout(function(){
      box.classList.remove('eta-fly','eta-leaving','eta-live');
      box.setAttribute('aria-hidden','true');
      live = false;
    }, 600);
  }
  ...
  hit.addEventListener('click', function(){ box.classList.toggle('eta-talking'); });
  hit.addEventListener('keydown', function(e){
    if (e.key === 'Enter' || e.key === ' '){ e.preventDefault(); box.classList.toggle('eta-talking'); }
  });
